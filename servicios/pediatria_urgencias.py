...
def extraer_drive_file_id_desde_link(link):
    if not link:
        return None
    patrones = [
        r"/d/([a-zA-Z0-9_-]+)",
    ...
    for patron in patrones:
        coincidencia = re.search(patron, str(link))
        if coincidencia:
...
def eliminar_historia_guardada(path, historia_id):
    ...
    drive_file_id = historia_objetivo.get("drive_file_id") or extraer_drive_file_id_desde_link(
        historia_objetivo.get("drive_webview_link")
    )
    drive_ok, drive_error = eliminar_archivo_drive(drive_file_id)
...
def construir_nombre_base_docx(tipo_archivo, nombre=None, documento=None, fecha_guardado=None, prefijo=None):
    ...
    return normalizar_nombre_archivo("_".join(partes))
...
def render():
    ...
    conducta_final_analisis = st.selectbox(
        ...
        on_change=lambda: st.session_state.__setitem__("analisis_actualizar_por_conducta", True),
    )
    ...
    registro_gpc_previo = str(st.session_state.get("gpc_registro", "") or "").strip()
    ruta_gpc_previa = st.session_state.get("gpc_ruta", "") or detectar_ruta_gpc(
        st.session_state.get("dx_cie10", ""),
    )
    analisis_default = ""
    ...
    elif permitir_generacion_analisis and (
        st.session_state.get("analisis_base") != analisis_default
        or st.session_state.pop("analisis_actualizar_por_conducta", False)
    ):
    ...
    ruta_gpc_sugerida = detectar_ruta_gpc(diagnostico_seleccionado, enfermedad_input)
